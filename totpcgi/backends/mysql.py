...
class GAUserConfigBackend(totpcgi.backends.GAUserConfigBackend):
    ...
    def get_user_config(self, user):
        ...
        logger.debug('Acquiring config lock for userid=%s' % userid)
        ...
        # the state backend
        cur.execute('SELECT GET_LOCK(-userconfig-%s,180)', (userid,))
        self.locks[user] = userid
    ...
    def update_user_config(self, user, gauc):
        ...
        logger.debug('Releasing config lock for userid=%s' % userid)
        cur.execute('SELECT RELEASE_LOCK(-userconfig-%s)', (userid,))
